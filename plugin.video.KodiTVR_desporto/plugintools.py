...
# Based on code from youtube, parsedom addons
#  
# 
# 
# THANKS TO ALL                                                  "@KodiTVR"
#---------------------------------------------------------------------------
# Changelog:
# 1.0.0
# - First release
...
def read_body_and_headers(url, post=None, headers=[], follow_redirects=False, timeout=None):
    ...
    # -------------------------------------------------
    # Cookies instalados, inicie o pedido
    # -------------------------------------------------
    ...
    if timeout is None:
        ...
        #Disponível em python 2.6 em diante --> handle = urlopen(req, timeout=timeout)
    ...
    # Atualize a loja de cookies
    if cj:  cj.save(ficherocookies)

    # Leia os dados e feche
    ...
    # Se falhar, ele o repete substituindo os caracteres especiais
    except:
