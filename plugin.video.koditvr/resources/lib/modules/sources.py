...
#                if not i['source'].lower() in self.hosthqDict and i['quality'] not in ['SD', 'SCR', 'CAM']: i.update({'quality': 'SD'})
    ...
    def getConstants(self):
        self.itemProperty = 'plugin.video.koditvr.container.items'

        self.metaProperty = 'plugin.video.koditvr.container.meta'
        ...
        oas_module_name = 'KodiTVRScrapers (' + str(control.addon('script.module.koditvrscrapers').getSetting('package.folder')) + ' set):' \
                          if control.addon('script.module.koditvrscrapers').getSetting('package.folder') != 'KodiTVRScrapers' else 'KodiTVRScrapers:'

        import koditvrscrapers
        sourceDir1 = koditvrscrapers.sources()
        ...
        try:
            if scraperSetting == 'KodiTVR Scrapers':
            ...
            elif scraperSetting == 'KodiTVR + Built-in':
                ...
                self.sourceDict = sourceDir1
                self.module_name = oas_module_name
                control.setSetting('module.provider', 'KodiTVR Scrapers')
        except:
            return
