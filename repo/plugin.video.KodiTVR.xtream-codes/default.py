...
addon_id = 'plugin.video.KodiTVR.xtream-codes'
...
livetv = \
    xbmc.translatePath(os.path.join('special://home/addons/plugin.video.KodiTVR.xtream-codes/resources/pic/livetv.png'
                       ))
filme = \
    xbmc.translatePath(os.path.join('special://home/addons/plugin.video.KodiTVR.xtream-codes/resources/pic/movie.png'
                       ))
serie = \
    xbmc.translatePath(os.path.join('special://home/addons/plugin.video.KodiTVR.xtream-codes/resources/pic/serie.png'
                       ))
suche = \
    xbmc.translatePath(os.path.join('special://home/addons/plugin.video.KodiTVR.xtream-codes/resources/pic/suche.png'
                       ))

username = control.setting('Username')
password = control.setting('Password')

live = control.setting('Host')
...
live_url = \
    '%s/enigma2.php?username=%s&password=%s&type=get_live_categories' \
    % (live, username, password)
vod_url = \
    '%s/enigma2.php?username=%s&password=%s&type=get_vod_categories' \
    % (live, username, password)
panel_api = '%s/panel_api.php?username=%s&password=%s' % (live,
        username, password)
panel_api1 = '%s/enigma2.php?username=%s&password=%s&type=get_series' % (live,
        username, password)
play_url = '%s/live/%s/%s/' % (live, username, password)
Series_url = \
    '%s/enigma2.php?username=%s&password=%s&type=get_series_categories' \
    % (live, username, password)

Guide = \
    xbmc.translatePath(os.path.join('special://home/addons/plugin.video.KodiTVR.xtream-codes/resources/catchup'
                       , 'guide.xml'))
GuideLoc = \
    xbmc.translatePath(os.path.join('special://home/addons/plugin.video.KodiTVR.xtream-codes/resources/catchup'
                       , 'g'))
...
def _pbhook(
    ...
    if dp.iscanceled():
        ...
        dialog.ok('IPTV', 'The download was cancelled.')
...
def stream_video(url):
    ...
    progress.create('Live TV', u'KodiTVR | The stream will start...')
    progress.update(15)
    ...
    try:
        while not abortReason:
            ...
            if step == 3:
                if player.getTime() > 0.01:
                    progress.update(100)
                    xbmcgui.Dialog().notification('Live TV by KodiTVR', 'Have fun with the stream', xbmcgui.NOTIFICATION_INFO, 2000, False)
                    break
            if not abortReason:
                xbmc.sleep(15)
        ...
        if abortReason:
            player.stop()
            xbmcgui.Dialog().ok('KodiTVR : The channel is not available', 'Please try again later.')
...
def searchdialog():
    search = control.inputDialog(heading='Search IPTV:')
    if search == '':
        return
    else:
        return search
...
def search():
	...
	text = searchdialog()
	if not text:
		xbmc.executebuiltin("XBMC.Notification(KodiTVR : Search field is empty,search canceled,4000,"+icon+")")
		return
	xbmc.log(str(text))
	open = tools.OPEN_URL(panel_api)
...
def addonsettings(url, description):
    if url == 'CC':
        ...
        if dialog == 0:
            ...
            xbmcgui.Dialog().ok('IPTV', 'Set Advanced Settings')
        elif dialog == 1:
        ...
        xbmc.executebuiltin('Runscript("special://home/addons/plugin.video.DWF-World1/resources/modules/speedtest.py")'
                            )
    elif url == 'META':
...
def correctPVR():
    ...
    addon = xbmcaddon.Addon('plugin.video.KodiTVR.xtream-codes')
