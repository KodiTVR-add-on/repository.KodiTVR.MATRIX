...
addonObject = addon('script.module.koditvrscrapers')
...
def autoTraktSubscription(tvshowtitle, year, imdb, tvdb):
    from koditvrscrapers.modules import libtools
...
def artPath():
    ...
    if theme in ['-', '']: return
        ...
        return os.path.join(xbmcaddon.Addon('script.koditvr.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.koditvr.artwork)') else setting('appearance.alt').lower()
    return appearance
...
def metaFile():
    if condVisibility('System.HasAddon(script.koditvr.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.koditvr.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
